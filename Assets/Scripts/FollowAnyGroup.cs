...
public class FollowAnyGroup : ReactiveBase
{
    public bool autoreact = false;
    public Register findTarget;
    ...
    private void OnValidate()
    {
        if (isActiveAndEnabled)
            if (findTarget == null)
                findTarget = GetComponentInParent<Register>();
    }
    ...
    public override bool React(Outputs outputs)
    {
        if (target == null && findTarget != null)
            target = findTarget.group.FindEnemy(targeting, transform.position);
